...
/** 組み込み + ユーザー追加カスタムフォントを統合した一覧を返す */
export function listAllFonts(): FontOption[] {
  // SSR / Node 環境では localStorage がないため、組み込みのみ
  if (typeof window === "undefined") return FONT_OPTIONS;
  // 循環依存を避けるため動的 import を発火させない:
  // ここでは window.__customFonts に毎回最新リストを書く方式 (font-loader が同期する)
  const w = window as Window & { __customFonts?: FontOption[] };
  return w.__customFonts ? [...FONT_OPTIONS, ...w.__customFonts] : FONT_OPTIONS;
}
